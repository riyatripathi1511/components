...
const HomePage = () => {
  ...
  //called on load of the component only once bcz dependency array is empty
  useEffect(() => {
    //getAllUsers();
    getAllUsersWithAwait();
  }, [ ]); //[] array helps to load the content only once
  ...
  const getAllUsers = () => {
    axios.get("http://localhost:5000/users")
      ...
      }) //do something is data is fetched from API successfully
      .catch(function (error) {
        console.log(error);
      }); //do something if there was an error while fetching data from API
  ...
  return (
    <div className="container">
     ...
     <div>
      ...
      <tbody>
        {  //inside jsx if we want to write javascript we will use {} braces
